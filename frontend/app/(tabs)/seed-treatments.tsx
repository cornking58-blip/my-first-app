...
const PesticideTypeBadge = ({ type }: { type: string | null }) => {
  if (type === 'fungicide_seed') {
    return (
      <View style={[styles.typeBadge, { backgroundColor: '#D1FAE5' }]}>
        <Text style={[styles.typeBadgeText, { color: '#059669' }]}>Фунгицидный</Text>
      </View>
    );
  }
  if (type === 'insecticide_seed') {
    return (
      <View style={[styles.typeBadge, { backgroundColor: '#FFF7ED' }]}>
        <Text style={[styles.typeBadgeText, { color: '#EA580C' }]}>Инсектицидный</Text>
      </View>
    );
  }
  return null;
...
export default function SeedTreatmentsScreen() {
  ...
  const { selectedSeedTreatmentsForCompare, toggleSeedTreatmentSelection, clearSeedTreatmentSelection } = useHerbicideStore();

  const fetchStats = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/stats`);
      if (response.data.seed_treatments) {
        setStats(response.data.seed_treatments);
      }
    } catch (error) {
  ...
  const search = async (query: string, active: boolean, cropValue: string, harmfulObjectValue: string) => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/api/seed-treatments/search?${params.toString()}`);
  ...
  const renderItem = ({ item }: { item: SearchResult }) => {
    ...
    const isSelected = selectedSeedTreatmentsForCompare.includes(item.product_key);
    const canSelect = selectedSeedTreatmentsForCompare.length < 2 || isSelected;
    
    return (
      <View style={[styles.card, isSelected && styles.cardSelected]}>
        <TouchableOpacity
          style={styles.cardContent}
          onPress={() => router.push(`/seed-treatment-product/${encodeURIComponent(item.product_key)}`)}
          activeOpacity={0.7}
        >
        ...
        <View style={styles.cardFooter}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => toggleSeedTreatmentSelection(item.product_key)}
            disabled={!canSelect && !isSelected}
          >
            <Ionicons 
              name={isSelected ? "checkmark-circle" : "add-circle-outline"} 
              size={18} 
              color={isSelected ? "#FFFFFF" : (!canSelect ? "#D1D5DB" : "#8B5CF6")} 
            />
            <Text style={[
      ...
      >
        <View style={styles.header}>
          <View style={styles.titleRow}>
            <View>
              <Logo />
              <Text style={styles.subtitle}>Справочник протравителей семян РФ</Text>
            </View>
          </View>
        ...
        <View style={styles.searchContainer}>
          ...
          <View style={styles.filterRow}>
            <TouchableOpacity
              style={[styles.filterButton, onlyActive && styles.filterButtonActive]}
            ...
            >
              ...
              />
              <Text style={[styles.filterText, onlyActive && styles.filterTextActive]}>Только действующие</Text>
            </TouchableOpacity>
        ...
        {selectedSeedTreatmentsForCompare.length > 0 && (
          <View style={styles.compareBar}>
            <View style={styles.compareInfo}>
              <Text style={styles.compareText}>Выбрано: {selectedSeedTreatmentsForCompare.length}</Text>
              <TouchableOpacity onPress={clearSeedTreatmentSelection}>
            ...
            {selectedSeedTreatmentsForCompare.length === 1 && (
...
const styles = StyleSheet.create({
  ...
  logoB: { color: '#F8F8FF' },
  logoAI: { color: '#9B7BFF', fontWeight: '900' },
  logoKov: { color: '#F8F8FF' },
  subtitle: { fontSize: 14, color: '#C8CBE0', marginTop: 2 },
  statsRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 16,
  ...
  statValue: { fontSize: 20, fontWeight: '700', color: '#F8F8FF' },
  ...
  compareText: { fontSize: 14, fontWeight: '600', color: '#5B21B6' },
  clearText: { marginLeft: 12, fontSize: 14, color: '#C8CBE0', textDecorationLine: 'underline' },
  compareButton: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#8B5CF6',
  ...
  compareButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '600', marginLeft: 6 },
  compareHint: { fontSize: 13, color: '#C8CBE0', fontStyle: 'italic' },
  ...
  productName: { fontSize: 17, fontWeight: '600', color: '#F8F8FF', flexShrink: 1 },
  formulationBadge: {
  ...
  formulationText: { fontSize: 12, color: '#C8CBE0', fontWeight: '500' },
  statusBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  statusActive: { backgroundColor: '#123C38' },
  statusInactive: { backgroundColor: '#421F35' },
  statusDot: { width: 6, height: 6, borderRadius: 3, marginRight: 6 },
  statusDotActive: { backgroundColor: '#10B981' },
  statusDotInactive: { backgroundColor: '#EF4444' },
  statusText: { fontSize: 11, fontWeight: '600' },
  statusTextActive: { color: '#48D6A5' },
  statusTextInactive: { color: '#FF718E' },
  typeBadge: {
  ...
  substances: { fontSize: 14, color: '#C8CBE0', lineHeight: 20, marginBottom: 8 },
  manufacturerRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  manufacturer: { fontSize: 13, color: '#969DBB', marginLeft: 6, flex: 1 },
  cardFooter: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  applicationsText: { fontSize: 13, color: '#969DBB', marginLeft: 6 },
  cardContent: { flex: 1 },
  compareSelectButton: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  compareSelectButtonActive: { backgroundColor: '#8B5CF6', borderColor: '#8B5CF6' },
  compareSelectButtonDisabled: { backgroundColor: '#1D2446', borderColor: '#2A335A' },
  compareSelectText: { fontSize: 12, fontWeight: '600', color: '#8B5CF6', marginLeft: 4 },
  compareSelectTextActive: { color: '#FFFFFF' },
  compareSelectTextDisabled: { color: '#969DBB' },
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingVertical: 60 },
  loadingText: { marginTop: 12, fontSize: 14, color: '#C8CBE0' },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingVertical: 60 },
  emptyTitle: { fontSize: 18, fontWeight: '600', color: '#F8F8FF', marginTop: 16 },
  emptyText: { fontSize: 14, color: '#969DBB', marginTop: 4 },
});
