...
export default function SeedTreatmentDetailScreen() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      const response = await axios.get(`${API_URL}/api/seed-treatments/${encodeURIComponent(key)}`);
                  ...
                  <View style={styles.applicationInfo}>
                    ...
                    {app.target_object && (
                      <Text style={styles.targetText} numberOfLines={expandedApp === index ? undefined : 1}>
                        Болезнь / вредитель: {app.target_object}
                      </Text>
                    )}
                  </View>
                </View>
