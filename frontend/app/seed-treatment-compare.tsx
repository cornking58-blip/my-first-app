...
export default function SeedTreatmentCompareScreen() {
  ...
  }, [routeParams.left_key, routeParams.right_key, selectedSeedTreatmentsForCompare]);
  ...
  const renderCanonicalSubstances = (substances: Substance[]) => (
    substances
      .map(substance => {
        const concentration = formatConcentration(substance.concentration, substance.unit);
        return concentration === '—' ? substance.name : `${concentration} ${substance.name}`;
      })
      .filter(Boolean)
      .join(' + ')
  );
