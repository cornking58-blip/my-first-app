...
export default function InsecticideCompareScreen() {
  const router = useRouter();
  const routeParams = useLocalSearchParams<{
    left_key?: string | string[];
    right_key?: string | string[];
  }>();
  const { selectedInsecticidesForCompare, clearInsecticideSelection } = useHerbicideStore();
  ...
  }, [routeParams.left_key, routeParams.right_key, selectedInsecticidesForCompare]);
  ...
  const fetchCompareData = async (withInputs = false) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/insecticides/compare-advanced`, body);
