...
WORKBOOK = ROOT / "fungicides_raw_FINAL.xlsx"
...
class FungicideSearchCompareFullParityTest(unittest.TestCase):
    def test_all_workbook_products_have_identical_search_and_compare_compositions(self):
        ...
        for source_record in read_xlsx_rows(WORKBOOK):
            ...
            record["pesticide_type"] = "fungicide"
            records_by_key[key].append(record)

        keys = list(records_by_key)
        self.assertEqual(len(keys), 478)
        self.assertEqual(sum(len(records) for records in records_by_key.values()), 1857)
        ...
        for key, records in records_by_key.items():
            ...
            raw_composition = first_parseable_composition(
