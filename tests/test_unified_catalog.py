...
STRICT_AI = (ROOT / "backend" / "strict_catalog_ai.py").read_text(encoding="utf-8")
...
class UnifiedCatalogStaticTest(unittest.TestCase):
    ...
    def test_ai_is_database_first(self):
        self.assertIn("return await build_strict_catalog_ai_context(db, message)", SERVER)
        self.assertIn('"verified_from_catalog": True', STRICT_AI)
        self.assertIn("Интернет-поиск не требуется", STRICT_AI)
        self.assertIn("build_strict_direct_answer(context)", SERVER)
        self.assertIn("def build_strict_direct_answer", STRICT_AI)
        self.assertIn('"allow_web_fallback": False', STRICT_AI)
    ...
    def test_product_names_are_found_with_inflection_and_context(self):
        ...
        self.assertIn("extract_single_product_candidate", STRICT_AI)
    ...
    def test_supabase_schema_and_migration_exist(self):
        ...
        self.assertTrue((ROOT / "backend" / "catalog_auto_migrate.py").exists())
